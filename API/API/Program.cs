...
// GET: Listar todas ou filtrar por classificação
app.MapGet("/api/pessoa/listar", ([FromServices] AppDataContext ctx, [FromQuery] string? classificacao) =>
{
    if (!string.IsNullOrEmpty(classificacao))
    {
        var filtradas = ctx.Pessoas.Where(p => p.Classificacao == classificacao).ToList();
        if (filtradas.Any())
            return Results.Ok(filtradas);
        return Results.NotFound($"Nenhuma pessoa encontrada com classificação '{classificacao}'");
    }
    if (ctx.Pessoas.Any())
    {
